...
for j = 1:N
  ...
  if plotting.verbosity > 0;      % visualization of trajectory
    ...
    for ii = 1:dimy
        ...
        [NMSE, mu, S2, NMLP, loghyper, convergence] = ssgprfixed_ui(X_tr, Y_tr(:, ii),  rand(x_len, dimx),...
                                                                                                       zeros(y_len, 1), nbf, -1000, loghyper);
    ...
    for mm =1:M_num
        ...
        states(:, 5:6) = states(:, 5:6) + [sin(states(:, angi)) cos(states(:, angi))] ;
        end
    ...
    % save all the data
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/X_data_" + num2str(j); 
    save(name, "X_tr")
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/Y_data_" + num2str(j); 
    save(name, "Y_tr")
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/trajectory_costs_" + num2str(j); 
    save(name, "trajectory_costs")
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/trajectories_" + num2str(j); 
    save(name, "trajectories");
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/uncertainty_" + num2str(j); 
    save(name, "uncertainty");
    name = "../../myData/cartpole_plots/data_" + num2str(save_at) +"/fantasy_data_" + num2str(j); 
    save(name, "fantasy");
end
