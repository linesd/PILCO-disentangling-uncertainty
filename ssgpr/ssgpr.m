...
function [out1, out2, out3] = ssgpr(optimizeparams, x_tr, y_tr, x_tst, posterior)
...
a = reshape(optimizeparams(D+3:end), [m, D]);                           % unscaled model angular frequencies
w = a./repmat(ell',[m,1]);                                              % scaled model angular frequencies

phi = x_tr*w';
phi = [cos(phi) sin(phi)];                                              % design matrix

K = (sf2/m)*(phi'*phi) + sn2*eye(2*m);
R = chol(K);                            % calculate some often-used constants
PhiRi=phi/R;
RtiPhit = PhiRi';
...
if nargout == 2
    ...
    A=[y_tr/sn2-PhiRi*((sf2/sn2/m)*Rtiphity) sqrt(sf2/sn2/m)*PhiRi];                                             % O(nm)
...
elseif nargin == 5
    ...
    out3 = phistar;
%     out1 = Rtiphity * (sf2 / m); %posterior mean
%     out2 = inv(K) * (sn2*sf2/m); % posterior covariance matrix
%     out3 = [cos(phistar) sin(phistar)]; % phistar
